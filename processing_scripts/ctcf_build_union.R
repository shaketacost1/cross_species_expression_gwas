...
meta_file   <- file.path(base_dir, "ctcf_report.tsv") # optional ENCODE report if you have it
dir.create(derived_dir, recursive = TRUE, showWarnings = FALSE)

# ---- helper: robust BED/Peak reader (gz or plain) ----
...
# ---- 3) optional: join metadata for tissue/cell labels ----
if (file.exists(meta_file)) {
  ...
  if (length(tcol)) {
    meta_small <- meta %>% dplyr::select(source, !!rlang::sym(tcol[1])) %>%
      dplyr::rename(tissue = !!rlang::sym(tcol[1]))
...
merged_dt <- data.table(
  chr   = as.character(GenomeInfoDb::seqnames(gr_merged)),
...
message("✅ Done: ", species, " — outputs in ", derived_dir)
message("  UNION : ", bed_union_path, " | ", tsv_union_path, " | ", parquet_union_path)
message("  MERGED: ", bed_merged_path,   " | ", parquet_merged_path)
